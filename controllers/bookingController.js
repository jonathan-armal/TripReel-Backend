...
exports.getBookingById = async (req, res) => {
    try {
        ...
        if (req.user.role !== 'admin' && String(booking.userId?._id || booking.userId) !== String(req.user._id)) {
...
exports.createBooking = async (req, res) => {
    try {
        const { listingId, travelers = [], travelStartDate, travelEndDate, guests } = req.body
        ...
        const template = listing.templateId ? await PackageTemplate.findById(listing.templateId).lean() : null

        const qty = Number(guests || (Array.isArray(travelers) && travelers.length ? travelers.length : 1)) || 1
